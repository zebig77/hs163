...
class TestDB extends TestHelper {

	@Test
	void test() {
		def f = new File('resources/data/hs_cards.json')
        ...
        .sort { it.name }
        .each {
            def cd = g.card_library.getCardDefinition(it.name)
	...
	@Test
	void testBeasts() {
		g.card_library.BEASTS_NAMES.eachLine { g.card_library.getCardDefinition(it) }
	}
}
